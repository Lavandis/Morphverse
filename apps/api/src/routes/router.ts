...
export async function routeRequest(request: IncomingMessage, response: ServerResponse) {
  ...
  try {
    if (method === "GET" && url.pathname === "/morphisms") {
      return sendJson(response, 200, morphismService.listMorphisms());
    }
    ...
    if (method === "POST" && url.pathname === "/morphisms") {
      ...
      return sendJson(response, 201, morphismService.createMorphism(input));
    }

    if (parts[0] === "morphisms" && parts[1] && method === "GET" && parts.length === 2) {
      return sendJson(response, 200, morphismService.getMorphism(parts[1]));
    }
    ...
    if (parts[0] === "morphisms" && parts[1] && parts[2] === "connections" && method === "POST") {
      ...
      return sendJson(
        response,
        200,
        morphismService.addConnection(parts[1], input.targetMorphismId)
      );
    }

    if (parts[0] === "morphisms" && parts[1] && parts[2] === "related-candidates" && method === "GET") {
      return sendJson(response, 200, morphismService.getRelatedCandidates(parts[1]));
    }

    if (parts[0] === "morphisms" && parts[1] && parts[2] === "compose-candidates" && method === "GET") {
      return sendJson(response, 200, morphismService.getComposeCandidates(parts[1]));
    }

    if (method === "POST" && url.pathname === "/composites") {
      const input = await readBody<{ sourceId: string; targetId: string }>(request);
      return sendJson(response, 201, morphismService.createComposite(input.sourceId, input.targetId));
    }
